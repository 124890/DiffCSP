...
class CDVAE(BaseModule):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.encoder_prior = hydra.utils.instantiate(self.hparams.prior, hidden_dim = self.hparams.latent_dim)
        ...
        self.fc_mu_prior = nn.Linear(self.hparams.latent_dim,
                               self.hparams.latent_dim)
        self.fc_var_prior = nn.Linear(self.hparams.latent_dim,
                                self.hparams.latent_dim)
    ...
    def encode_prior(self, batch):
        hidden = self.encoder_prior(batch)
        mu = self.fc_mu_prior(hidden)
        log_var = self.fc_var_prior(hidden)
        z = self.reparameterize(mu, log_var)
        return mu, log_var, z        
    ...
    @torch.no_grad()
    def langevin_dynamics(self, z, ld_kwargs, gt_num_atoms=None, gt_atom_types=None, gt_lattice=None):
        """
        ...
        if gt_lattice is not None:
            lengths, angles = gt_lattice[:,:3], gt_lattice[:,3:]
    ...
    def sample(self, batch, diff_ratio = 1.0, step_lr=1e-4):
        ld_kwargs = SimpleNamespace(n_step_each=100,
                                    ...
                                    save_traj=True,
                                    disable_bar=False)
        _, _, z = self.encode_prior(batch)
        samples = self.langevin_dynamics(z, ld_kwargs, gt_num_atoms=batch.num_atoms, gt_atom_types=batch.atom_types)
        ...
        s0 = {
            ...
            'lattices' : lattice_params_to_matrix_torch(samples['lengths'], samples['angles'])
        }

        traj = {
            'num_atoms' : samples['num_atoms'],
            'atom_types' : samples['all_atom_types'],
            'all_frac_coords' : samples['all_frac_coords'],
            'all_lattices' : s0['lattices'][None,:].repeat(samples['all_frac_coords'].shape[0], *[1] * len(s0['lattices'].shape))
        }
    ...
    def forward(self, batch, teacher_forcing, training):
        ...
        mu_prior, log_var_prior, _ = self.encode_prior(batch)
        ...
        kld_loss = self.kld_loss(mu, log_var, mu_prior, log_var_prior)
    ...
    def kld_loss(self, mu, log_var, mu_prior, logvar_prior):

        kld_loss = torch.mean(
            -0.5 * torch.sum(1 + log_var - logvar_prior - (mu - mu_prior)**2 / logvar_prior.exp() - (log_var - logvar_prior).exp(), dim=1), dim=0)
        return kld_loss
